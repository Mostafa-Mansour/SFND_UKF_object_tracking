...
#define EPS 0.0001 // small error number to avoid ploating point problems
/**
...
UKF::UKF():is_initialized_(false), use_laser_(true), use_radar_(true) {
  ...
  n_aug_ = n_x_ + 2; //  2 * n_aug_ + 1 sigma points 
...
void UKF::ProcessMeasurement(MeasurementPackage& meas_package) {
  ...
  // Initial covariance matrix
    ...
    if (meas_package.sensor_type_ == MeasurementPackage::RADAR) {
      ...
      // no information about the velocity, yaw angle, and yaw rate
...
void UKF::UpdateRadar(MeasurementPackage& meas_package) {
  ...
  for (int i = 0; i < n_sig_; i++) {
    ...
    Zsig(2,i) = (p_x*v1 + p_y*v2 ) / Zsig(0,i);   //vel
  }
  UpdateUKF(meas_package, Zsig, n_z);
}
...
void UKF::UpdateUKF(MeasurementPackage& meas_package, MatrixXd Zsig, int n_z){
  ...
  // Calculate NIS if needed to check the consistency of the filter
  if (meas_package.sensor_type_ == MeasurementPackage::RADAR){ // Radar
